...
{
 Vector v1;
 memcpy(v1, globalField.nodes[n1].location, sizeof(Vector));

 Vector v2;
 memcpy(v2, globalField.nodes[n2].location, sizeof(Vector));
...
void FieldRecalculatePath()
{
	...
	for ( Node n = globalField.targetNode; n != NodeZero; n = previous[n] ) //	iterate through the path until we hit the beginning
	{
...
{
	if ( !globalField.cached ) _FieldFindShortestPath(globalField.cachedPath[0], globalField.goalNode);	//	recalculate if necessary
