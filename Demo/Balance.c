...
#define InRange(val, min, max) (val <= max && val >= min)
...
task main()
{
  ...
  short x;
  wait10Msec(200);
  HTACreadX(Accelerometer, x);



 while ( !InRange(x, kMinTilt, kMaxTilt) )  //  while we're not balanced
 {
    nxtDisplayCenteredTextLine(0, (string)x);


    float error = (float)kBalancedTilt - (float)x;
    ...
    /*
    if ( x > kMaxTilt )  //  we're too far back
    {
